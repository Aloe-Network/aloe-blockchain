...
from aloe.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from aloe.full_node.full_node_api import FullNodeAPI
from aloe.server.start_farmer import service_kwargs_for_farmer
from aloe.server.start_full_node import service_kwargs_for_full_node
from aloe.server.start_harvester import service_kwargs_for_harvester
from aloe.server.start_introducer import service_kwargs_for_introducer
from aloe.server.start_service import Service
from aloe.server.start_timelord import service_kwargs_for_timelord
from aloe.server.start_wallet import service_kwargs_for_wallet
from aloe.simulator.start_simulator import service_kwargs_for_full_node_simulator
from aloe.timelord.timelord_launcher import kill_processes, spawn_process
...
from aloe.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["aloe_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "aloe")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["aloe_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "aloe")
