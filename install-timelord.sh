...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the aloe python virtual environment."
...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("aloevdf").location)' 2>/dev/null)/vdf_client
ALOEVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("aloevdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing aloevdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary aloevdf "$ALOEVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary aloevdf "$ALOEVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing aloevdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing aloevdf requirement boost for MacOS."
		brew install boost
		echo "Installing aloevdf from source."
