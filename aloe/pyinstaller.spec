...
ROOT = pathlib.Path(importlib.import_module("aloe").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `aloe` file name (which is the executable)
    and the `aloe` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `aloe` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("aloe-blockchain"))[0]
...
# TODO: collapse all these entry points into one `aloe_exec` entrypoint that accepts the server as a parameter

entry_points = ["aloe.cmds.aloe"] + [f"aloe.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "aloe.util.bip39"])

if THIS_IS_WINDOWS:
    aloe_mod = importlib.import_module("aloe")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/aloe/util/english.txt", "aloe/util"))
datas.append((f"{ROOT}/aloe/util/initial-config.yaml", "aloe/util"))
datas.append((f"{ROOT}/aloe/wallet/puzzles/*.hex", "aloe/wallet/puzzles"))
datas.append((f"{ROOT}/aloe/ssl/*", "aloe/ssl"))
...
add_binary("aloe", f"{ROOT}/aloe/cmds/aloe.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/aloe/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/aloe/server/start_{server}.py", COLLECT_ARGS)
