...
from aloe.cmds.init_funcs import aloe_init
from aloe.daemon.windows_signal import kill
from aloe.server.server import ssl_context_for_root, ssl_context_for_server
from aloe.ssl.create_ssl import get_mozilla_ca_crt
...
from aloe.util.service_groups import validate_service
from aloe.util.setproctitle import setproctitle
from aloe.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Aloe.")
    quit()
...
service_plotter = "aloe plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "aloe": "aloe",
        "aloe_wallet": "start_wallet",
        "aloe_full_node": "start_full_node",
        "aloe_harvester": "start_harvester",
        "aloe_farmer": "start_farmer",
        "aloe_introducer": "start_introducer",
        "aloe_timelord": "start_timelord",
        "aloe_timelord_launcher": "timelord_launcher",
        "aloe_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("aloe daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered ALOE_ROOT
    os.environ["ALOE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with ALOE_ROOT: {os.environ['ALOE_ROOT']}")
