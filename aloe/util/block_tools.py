...
from aloe.cmds.init_funcs import create_all_ssl, create_default_aloe_config
from aloe.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from aloe.plotting.create_plots import create_plots
from aloe.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
from aloe.consensus.block_record import BlockRecord
from aloe.consensus.blockchain_interface import BlockchainInterface
from aloe.consensus.coinbase import create_puzzlehash_for_pk
from aloe.consensus.constants import ConsensusConstants
from aloe.consensus.default_constants import DEFAULT_CONSTANTS
from aloe.consensus.deficit import calculate_deficit
from aloe.consensus.full_block_to_block_record import block_to_block_record
from aloe.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from aloe.consensus.pot_iterations import (
    calculate_ip_iters,
...
from aloe.consensus.vdf_info_computation import get_signage_point_vdf_info
from aloe.full_node.signage_point import SignagePoint
from aloe.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from aloe.types.blockchain_format.proof_of_space import ProofOfSpace
from aloe.types.blockchain_format.sized_bytes import bytes32
from aloe.types.blockchain_format.slots import (
...
from aloe.types.generator_types import BlockGenerator, CompressorArg
...
from aloe.util.config import load_config, save_config
from aloe.util.hash import std_hash
from aloe.util.ints import uint8, uint32, uint64, uint128
from aloe.util.keychain import Keychain, bytes_to_mnemonic
from aloe.util.path import mkdir
from aloe.util.vdf_prover import get_vdf_info_and_proof
from aloe.util.wallet_tools import WalletTool
from aloe.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `aloe generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "aloe")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("ALOE_ROOT", "~/.aloe/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
