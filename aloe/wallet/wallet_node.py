...
from aloe.protocols import wallet_protocol
from aloe.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from aloe.protocols.protocol_message_types import ProtocolMessageTypes
from aloe.protocols.wallet_protocol import (
...
from aloe.util.errors import Err, ValidationError
from aloe.util.ints import uint32, uint128
from aloe.util.keychain import Keychain
from aloe.util.lru_cache import LRUCache
from aloe.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from aloe.util.path import mkdir, path_from_root
from aloe.wallet.block_record import HeaderBlockRecord
from aloe.wallet.derivation_record import DerivationRecord
from aloe.wallet.settings.settings_objects import BackupInitialized
from aloe.wallet.transaction_record import TransactionRecord
from aloe.wallet.util.backup_utils import open_backup_file
from aloe.wallet.util.wallet_types import WalletType
from aloe.wallet.wallet_action import WalletAction
from aloe.wallet.wallet_blockchain import ReceiveBlockResult
from aloe.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'aloe keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSAloeConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSAloeConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSAloeConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSAloeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSAloeConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSAloeConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
